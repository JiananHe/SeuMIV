...
{
	breakpoints_map.insert(pair<double, T>(key, interpolateValue(key)));
}

template<typename T>
inline T BreakPoints<T>::interpolateValue(double key)
{
	...
	return value_small + (value_big - value_small) * r;
}
